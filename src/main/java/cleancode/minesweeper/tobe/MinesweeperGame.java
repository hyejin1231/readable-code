...
public class MinesweeperGame {
    ...
    private static void actOnCell(String cellInput, String userActionInput) {
        ...
        throw new AppException("잘못된 번호를 선택하셨니다.");
    }
    ...
    private static void checkIfGameOver() {
        boolean isAllOpend = isAllCellOpened();
        if (isAllOpend) {
    ...
    // isAllCellOpened 리팩토링 전
    private static boolean isAllCellOpene2() {
        boolean isAllOpend = true; // 전체 board 가 다 열렸는가
    ...
    // isAllCellOpened 리팩토링
    private static boolean isAllCellOpened() {
        return Arrays.stream(BOARD)
                .flatMap(Arrays::stream)
                .noneMatch(CLOSED_CELL_SIGN::equals); // 상수는 null 일 확률이 적으니까 상수의 equals를 호출하는게 좋다.
    }
    ...
    private static void initializeGame() {
        for (int row = 0; row < BOARD_ROW_SIZE; row++) { // i -> row
            for (int col = 0; col < BOARD_COL_SIZE; col++) { // j -> col
    ...
    private static int countNearByLandMines(int row, int col) {
        int count = 0; // 입력한 칸 주변에 지뢰가 몇개인지 count
